...
class DetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 좋아요 버튼의 변경을 확인 하기 위한 변수
        var changeImage = true
        var count: Int = item?.dHeart ?: 0          // 이건 좋아요 개수 인데 DetailPage 에는 표시 하지 않으니 여기로 따로 빼놓음

        // 좋아요 버튼을 눌렀을 때 좋아요 버튼 이미지, 좋아요 수를 결정 하는 부분
        binding.ivBottomHeart.setOnClickListener { view ->
            if (changeImage) {                                                          // 처음 상태 -> 이미지 누르면 빨간 하트 이미지 변환, 좋아요 수 + 1
                ...
                Snackbar.make(view, "관심 목록에 추가\n좋아요 개수 : $count", Snackbar.LENGTH_SHORT)
                    .show()
            } else {                                                                      // 빨간 하트 상태 -> 이미지 누르면 빈 하트 이미지 변환, 좋아요 수 -1
        ...
        // 뒤로 가기 버튼 누르면 바뀐 DataList 들고 MainPage로 이동
